...
func (p *Processor) ProcessCharacterArt(art *CharacterArt) ([]CharacterArtImage, error) {
	...
	for i, body := range art.BodyVariations {
		...
		// Thanks to HyperGryph ^^
		if bodyImage == nil {
			continue
		}

		if body.PixelToUnits != 1.0 {
			bodyImage = imaging.Resize(
				bodyImage,
				int(math.Round(float64(bodyImage.Bounds().Dx())/body.PixelToUnits)),
				int(math.Round(float64(bodyImage.Bounds().Dy())/body.PixelToUnits)),
				imaging.Lanczos,
			)
		}
		...
		for j, face := range body.FaceVariations {
			...
			if face.PixelToUnits != 1.0 {
				faceImage = imaging.Resize(
					faceImage,
					int(math.Round(float64(faceImage.Bounds().Dx())/face.PixelToUnits)),
					int(math.Round(float64(faceImage.Bounds().Dy())/face.PixelToUnits)),
					imaging.Lanczos,
				)
			}
